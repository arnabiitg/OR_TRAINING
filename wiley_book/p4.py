...
for month in months:    
    ...
    for machine in machines:
        machine_used[(machine, month)] = solver.BoolVar(f"machine_used_{machine}{month}")

# Define the constraints
# Machine Usage Constraint
for machine in machines[1:]:
    solver.Add(sum([machine_used[(machine,month)] for month in months]) == n_machines[machine])
solver.Add(sum([machine_used[(0,month)] for month in months]) == 2)
# Time Constraint
for month in months:
    for machine in machines:
        solver.Add(sum([produced[(prod,month)]*time[machine][prod] for prod in prods]) <= (n_machines[machine] - machine_used[(machine,month)])*24*16)
